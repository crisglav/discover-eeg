...
% params.study = 'LEMON-open';
% 
% % The path of your raw data in BIDS format
% params.raw_data_path = '/rechenmagd4/Experiments/2021_preprocessing/datasets/LEMON-8min-open-bids';
% 
...
% params.session = {}; % For all sessions: {} / for a specific session e.g. {'baseline'}
% params.runs = [];
% params.task = []; % For all tasks: [] / for a specific task e.g. 'closed'
% 
% % EEGLab path
...
% if ~exist(params.figufname = 'params.json';
% fid = fopen(fname);
% raw = fread(fid,inf);
% str = char(raw');
% fclose(fid);
% params = jsondecode(str);res_preprocessing_folder,'dir')
